...
function formatDuration(seconds) {
  ...
  for (let value of timeFormat) {
    if (seconds >= value[1]) {
      const number = Math.floor(seconds / value[1]);
      number === 1
        ? arr.push(`${number} ${value[0]}`)
        : arr.push(`${number} ${value[0]}s`);
      seconds = seconds % value[1];
    }
  }

  for (let value of arr) {
    const where = arr.findIndex((number) => number === value);
    if (where === arr.length - 1 || value === " and " || value === ", ") {
      continue;
    } else if (where !== arr.length - 2) {
      arr.splice(where + 1, 0, ", ");
    } else {
      arr.splice(where + 1, 0, " and ");
    }
  }
