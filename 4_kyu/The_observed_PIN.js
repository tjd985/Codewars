...
// but he also said, it is possible that each of the digits he saw could actually be another adjacent digit(horizontally or vertically,
// but not diagonally).
...
function getPINs(observed) {
  ...
  for (let value of observed) {
    formatList.push(numberList[value]);
  }

  function makePassword(arr, possible, depth) {
    for (let i = 0; i < arr.length; i++) {
      possible.push(arr[i]);

      if (depth === formatList.length - 1) {
        const complete = possible.join("");
        password.push(complete);
        possible.pop();
        continue;
      }
      makePassword(formatList[depth + 1], possible, depth + 1);
      possible.pop();
    }
  }
  makePassword(formatList[0], [], 0);
